<?php
/**
 * Opalmembership Session
 *
 * This is a wrapper class for WP_Session / PHP $_SESSION and handles the storage of Opalmembership sessions
 *
...
class Opalmembership_Session {
	...
	public function init() {
		...
		$cart       = $this->get( 'opalmembership_cart' );
		$purchase   = $this->get( 'opalmembership_purchase' );
	...
	public function set_cart_cookie( $set = true ) {
		if( ! headers_sent() ) {
			if( $set ) {
				@setcookie( 'opalmembership_items_in_cart', '1', time() + 30 * 60, COOKIEPATH, COOKIE_DOMAIN, false );
			} else {
				if ( isset($_COOKIE['opalmembership_items_in_cart']) ) {
					@setcookie( 'opalmembership_items_in_cart', '', time() - 3600, COOKIEPATH, COOKIE_DOMAIN, false );
				}
			}
		}
	}
	...
	/**
	 * Determines if a user has set the Opalmembership_USE_CART_COOKIE
	 *
	 * @since  2.5
	 * @return bool If the store should use the opalmembership_items_in_cart cookie to help avoid caching
	 */
	public function use_cart_cookie() {
		$ret = true;

		if ( defined( 'Opalmembership_USE_CART_COOKIE' ) && ! Opalmembership_USE_CART_COOKIE ) {
			$ret = false;
		}

		return (bool) apply_filters( 'opalmembership_use_cart_cookie', $ret );
	}
	...
	public function should_start_session() {
		...
		return apply_filters( 'opalmembership_start_session', $start_session );
	...
	public function get_blacklist() {
		...
		$blacklist = apply_filters( 'opalmembership_session_start_uri_blacklist', array(
			'feed',
			'feed/rss',
			'feed/rss2',
			'feed/rdf',
