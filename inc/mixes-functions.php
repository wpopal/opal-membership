...
function opalmembership_get_payment_note_html( $note, $payment_id = 0 ) {
    ...
    $note_html .= '&nbsp;&ndash;&nbsp;<a href="' . esc_url( $delete_note_url ) . '" class="opalmembership-delete-payment-note" data-note-id="' . absint( $note->comment_ID ) . '" data-payment-id="' . absint( $payment_id ) . '" title="' . esc_html__( 'Delete this payment note', 'opalmembership' ) . '">' . esc_html__( 'Delete', 'opalmembership' ) . '</a>';
    $note_html .= '</p>';
    $note_html .= '</div>';
...
if ( ! function_exists( 'opalmembership_print_login_form' ) ) {
    function opalmembership_print_login_form( $args = array() ) {
        $args = wp_parse_args( $args, array(
                'message'   => esc_html__( 'If you have registed with us before, please enter your details in the boxes below. If you are a new agent, please proceed to the Billing section.', 'opalmembership' ),
                'redirect'  => '',
                'hide_title'    => true
            ) );
